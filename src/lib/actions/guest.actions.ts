...
import { AttendMode, EventType, GuestStatus, Role, Tier, ZoomSessionKind } from "@prisma/client";
...
import { getJoinPageAbsoluteUrl, getOpenZoomJoinAbsoluteUrl } from "@/lib/url";
...
async function getEventForGuestAction(eventId: string, orgId: string) {
  return prisma.event.findFirst({
    ...
    include: { location: true, org: { select: { name: true, resendApiKey: true } } }
  });
}
...
async function createGuestWithSideEffects(
  ...
  if (isMeetingVirtual && !event.zoomJoinUrl) {
    return {
      success: false,
      error:
  ...
  const initialZoomLink: string | null = isMeetingVirtual ? event.zoomJoinUrl : null;
  ...
  try {
    ...
    return { success: true, data: { ...formattedGuest, emailDelivered } };
  } catch {
...
export async function addGuest(
  ...
  if (isRepScopedRole(session.user.role)) {
...
async function sendGuestEmailsAfterCreate(
  ...
  if (guest.mode === AttendMode.VIRTUAL && zoomLink && event.zoomMeetingId) {
    ...
    await sendGuestConfirmationVirtual({
      ...
      zoomJoinUrl: perGuestOpenZoom ?? zoomLink,
...
export async function importGuestsFromRows(
  ...
  const reps = await prisma.user.findMany({
    where: { orgId: session.user.orgId, role: { in: [Role.STAFF, Role.SALES_REF] } },
...
export async function updateGuestStatus(
  ...
  if (isRepScopedRole(session.user.role) && existing.repId !== session.user.id) {
    return { success: false, error: "Unauthorized" };
  }

...
type GuestFormProps = {
  ...
  onSuccess: (opts: { emailDelivered: boolean }) => void;
  onCancel: () => void;
};
...
export function GuestForm({ eventId, salesReps, role, currentUserId, onSuccess, onCancel }: GuestFormProps) {
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      dietary: "",
      repId: isRepScopedRole(role) ? currentUserId : ""
    }
  });
  ...
  async function onSubmit(values: FormValues) {
    const repId = isRepScopedRole(role)
      ? currentUserId
      : values.repId?.trim()
        ? values.repId.trim()
        : undefined;
    ...
    onSuccess({ emailDelivered: res.data.emailDelivered });
  }

...
import { canExportGuestData } from "@/lib/permissions";
...
export function GuestManagementPanel({
  ...
  function afterAdd(opts: { emailDelivered: boolean }) {
    setAddOpen(false);
    if (!opts.emailDelivered) {
      window.alert(
        "Guest was saved, but the confirmation email was not sent. Check Resend under Settings → Integrations."
      );
    }
    window.location.reload();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start">
        <div className="flex flex-wrap items-start gap-2">
          ...
          {showZoomParticipantSync ? (
            <ZoomParticipantSyncPanel eventId={eventId} layout="toolbar" />
          ) : null}
        </div>
        <div className="flex flex-wrap gap-2 sm:ml-auto">
          {canExportGuestData(role) ? (

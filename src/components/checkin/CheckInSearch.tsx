...
type CheckInSearchProps = {
  ...
  disabled?: boolean;
  offline?: boolean;
  cachedGuests?: CachedGuestRow[];
};

export function CheckInSearch({ eventId, onPickGuest, disabled, offline, cachedGuests }: CheckInSearchProps) {
  ...
  const runSearch = useCallback(async () => {
    ...
    if (offline) {
      ...
      if (!cachedGuests?.length) {
        setResults([]);
        setError("No offline guest list yet. Connect once on this device to download guests.");
        return;
      }
      const matches = cachedGuests.filter(
        (g) => g.name.toLowerCase().includes(q) || g.email.toLowerCase().includes(q)
      );
      setResults(matches.map((g) => ({ id: g.id, name: g.name, email: g.email })));
      return;
    }
    setLoading(true);
  ...
  }, [cachedGuests, eventId, offline, query]);
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        {offline
          ? "Offline mode: results come from the last synced guest list on this device."
          : "Find a guest by name or email, then check them in."}
      </p>
